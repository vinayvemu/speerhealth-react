...
import { Box, Typography, Grid, Paper, Button, CircularProgress, LinearProgress, Alert } from '@mui/material';
...
function KpiCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <Paper elevation={0} sx={{ p: 2, border: '1px solid #E3E7FF', borderRadius: 2, textAlign: 'center' }}>
      ...
      {sub && <Typography variant="caption" color="success.main" fontWeight={600}>{sub}</Typography>}
    </Paper>
  );
}
  ...
  return (
    <Box sx={{ overflowY: 'auto', height: '100%', p: 2, pb: 4 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6" fontWeight={800} color="#1A237E">Analytics</Typography>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid #E3E7FF', borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} color="#1A237E" sx={{ mb: 1.5 }}>Pipeline Funnel</Typography>
      ...
      {/* Insights Over Time */}
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid #E3E7FF', borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} color="#1A237E" sx={{ mb: 1.5 }}>Insights Over Time (8 weeks)</Typography>
        <ComponentErrorBoundary>
          <LazyChart>
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={data?.weekly ?? []}>
                ...
                <Tooltip />
                <Line type="monotone" dataKey="count" stroke="#3F51B5" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </LazyChart>
        </ComponentErrorBoundary>
      ...
      {/* Category Bar */}
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid #E3E7FF', borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} color="#1A237E" sx={{ mb: 1.5 }}>By Category</Typography>
        <ComponentErrorBoundary>
          <LazyChart>
            <ResponsiveContainer width="100%" height={160}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid #E3E7FF', borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} color="#1A237E" sx={{ mb: 1.5 }}>Priority by Stage</Typography>
        <ComponentErrorBoundary>
          <LazyChart>
            <Box sx={{ overflowX: 'auto' }}>
              <Box sx={{ display: 'grid', gridTemplateColumns: 'auto repeat(4, 1fr)', gap: 0.5, minWidth: 300 }}>
                ...
                {(Object.keys(STAGE_LABELS) as Stage[]).map((s) => (
                  <Typography key={s} variant="caption" fontWeight={600} color="text.secondary" sx={{ textAlign: 'center' }}>
                    {STAGE_LABELS[s].slice(0, 3)}
                  </Typography>
                ))}
                {(['P1', 'P2', 'P3', 'P4'] as const).map((p) => (
                  <React.Fragment key={p}>
                    <Typography variant="caption" fontWeight={600} sx={{ color: PRIORITY_COLORS[p].text, display: 'flex', alignItems: 'center' }}>
                      {p}
                    </Typography>
                    {(Object.keys(STAGE_LABELS) as Stage[]).map((s) => {
                      ...
                      return (
                        ...
                        >
                          <Typography variant="caption" fontWeight={count > 0 ? 700 : 400} color={count > 0 ? '#1A237E' : 'text.disabled'}>
                            {count}
                          </Typography>
                        </Box>
                      );
      ...
      {/* HCP Leaderboard */}
      <Paper elevation={0} sx={{ p: 2, mb: 2, border: '1px solid #E3E7FF', borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} color="#1A237E" sx={{ mb: 1.5 }}>Top HCPs</Typography>
        <ComponentErrorBoundary>
          <LazyChart>
            <ResponsiveContainer width="100%" height={160}>
